...
export function LandingHero() {
  return (
    <>
      <section className="relative overflow-hidden border-b border-[#292721]">
        ...
        <div className="flex flex-col justify-between border-[#292721] px-6 pb-12 pt-16 sm:px-10 sm:pt-24 lg:border-r lg:px-16 lg:pb-16 xl:px-20">
          <div>
            ...
            <div className="reveal mt-5 flex flex-wrap gap-x-5 gap-y-2 font-mono text-[8px] uppercase tracking-[.14em] text-[#6d675f] [animation-delay:300ms]">
      ...
      <section aria-label="RepoMind operating principles" className="border-b border-[#292721] bg-[#e8dfcf]">
        <div className="mx-auto grid max-w-[90rem] sm:grid-cols-2 lg:grid-cols-4">
          {[
            ["01", "Public input", "A GitHub repository or branch URL."],
            ["02", "Structural signals", "Files, metadata, and detected relationships."],
            ["03", "AI-assisted context", "Explanations grounded in repository signals."],
            ["04", "Practical output", "A saved report and ordered route in."],
          ].map(([number, title, body], index) => (
            <div key={number} className={`px-6 py-5 sm:px-8 ${index < 3 ? "border-b border-[#292721] lg:border-b-0 lg:border-r" : ""} ${index === 0 ? "sm:border-r" : ""} ${index === 2 ? "sm:border-b-0 sm:border-r" : ""}`}>
              <div className="flex items-center gap-3">
                <span className="font-mono text-[8px] tracking-[.15em] text-[#c94f34]">{number}</span>
                <p className="text-sm font-medium">{title}</p>
              </div>
              <p className="mt-2 pl-8 text-xs leading-5 text-[#6d675f]">{body}</p>
            </div>
          ))}
        </div>
      </section>
